                    ...
                    for(size_t sampleIdx = 0; sampleIdx < maxTimeSamples; sampleIdx++)
                    {
                        ...
                        for(size_t wireIdx = 0; wireIdx < nWiresPerMotherBoard; wireIdx++)
                        {
                            ...
                            if (rmsNoise > fRmsRejectionCutLow[viewIdx] && rmsNoise < fRmsRejectionCut[viewIdx])
                            {
                                ...
                                if (fabs(adcLessPed) < 10.*rmsNoise) adcValuesMap[adcValue]++;
                            }
                        }
                        ...
                        for(size_t wireIdx = 0; wireIdx < nWiresPerMotherBoard; wireIdx++)
                        {
                            ...
                            if (rmsNoise > fRmsRejectionCutLow[viewIdx] && rmsNoise < fRmsRejectionCut[viewIdx])
                            {
                                ...
                                float     newAdcValueFloat = float(rawDataViewWireTimeVec[viewIdx][physWireIdx][sampleIdx]) - mostProbableValue - pedCorViewWireVec[viewIdx][physWireIdx];
